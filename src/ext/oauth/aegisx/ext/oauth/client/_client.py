...
class Client(httpx.AsyncClient):
    ...
    async def obtain(
        ...
        return await self.grant(self.metadata.token_endpoint, grant)

    async def grant(self, endpoint: str, grant: TokenRequest):
        self._select_client_secret_jwt_alg(self.metadata)
        response = await self.post(
            url=endpoint,
            json=grant.model_dump(
                mode='json',
                exclude_unset=True,
                exclude_defaults=True,
                exclude_none=True
    ...
    async def on_redirected(
        ...
        response = AuthorizationResponse.model_validate(result)
        return response
