...
class ServerMetadata(pydantic.BaseModel):
    ...
    jwks: JSONWebKeySet | None = pydantic.Field(
        default=None,
        exclude=True
    )
    ...
    async def discover(
        ...
        async with self._client(client) as client:
            ...
            if response is None:
                raise ValueError(f"Unable to discover server metadata for {self.issuer}")
            try:
    ...
    async def _client(self, client: httpx.AsyncClient | None):
        if client is not None:
            yield client
            return
        async with httpx.AsyncClient(base_url=self.issuer) as client:
            yield client
